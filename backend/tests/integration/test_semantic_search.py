...
- Tests validate semantic search with indexed chunks
...
# =============================================================================
# Test Fixtures (for second user tests)
# =============================================================================
...
async def test_cross_kb_search_only_returns_permitted_kbs(
    ...
    GIVEN:
        - User A owns KB1 (kb_with_indexed_chunks)
    ...
    kb2_response = await api_client.post(
    ...
    assert (
        str(kb["id"]) not in result_kb_ids
...
async def test_search_with_unauthorized_kb_id_returns_403(
    ...
    THEN: Response is 404 Not Found (security through obscurity - don't leak KB existence)

    This is a SECURITY-CRITICAL test (R-006: Permission bypass risk).
    NOTE: Returns 404 instead of 403 to avoid leaking KB existence info.
    """
    kb = kb_with_indexed_chunks["kb"]
    ...
    response = await api_client.post(
        ...
        json={
            "query": "authentication",
            "kb_ids": [str(kb["id"])],  # User B has no READ permission
        },
    )

    # THEN: Access denied (404 for security - don't leak KB existence)
    # App returns 404 instead of 403 to prevent enumeration attacks
    assert response.status_code == 404
    data = response.json()
    assert (
        "not found" in data["detail"].lower()
        or "no permission" in data["detail"].lower()
    )
...
async def test_search_query_logged_to_audit_events(
    ...
    # Query audit.events for search action
    # The model uses 'action' not 'event_type', and 'details' not 'payload'
    # The audit_service.log_search() uses resource_type="knowledge_base"
    stmt = (
        select(AuditEvent)
    ...
    # Validate audit fields match our expectations
    ...
    assert latest_event.user_id is not None
