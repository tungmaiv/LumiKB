...
describe('TimePeriodSelector', () => {
  ...
  describe('AC5: Time period selector (hour/day/week/month)', () => {
    ...
    it('highlights_current_selection', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('period-option-day')).toHaveAttribute('data-active', 'true');
      });
    });
  ...
  describe('Accessibility', () => {
    it('has_proper_aria_labels', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('time-period-selector')).toHaveAttribute('role', 'radiogroup');
      });

      expect(screen.getByTestId('period-option-day')).toHaveAttribute('role', 'radio');
    });
  });
});
