...
describe('LLMUsageWidget', () => {
  ...
  describe('AC1: Displays total tokens and cost', () => {
    it('renders_total_tokens_and_cost', async () => {
      ...
      const LLMUsageWidget = await import('../llm-usage-widget').then((m) => m.LLMUsageWidget);
      ...
      expect(screen.getByTestId('llm-total-tokens')).toHaveTextContent('125,000');
    ...
    it('formats_large_numbers_correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('llm-total-tokens')).toHaveTextContent('1,250,000');
      });
    });
  });
  ...
  describe('Loading and error states', () => {
    ...
    it('shows_error_state_on_error', async () => {
      ...
      renderWithProviders(<LLMUsageWidget data={null} period="day" error="Failed to load" />);
