...
describe('ChatActivityWidget', () => {
  ...
  describe('AC3: Messages today, active sessions, avg response time', () => {
    ...
    it('renders_active_sessions', async () => {
      ...
      expect(screen.getByTestId('chat-active-sessions')).toHaveTextContent('45');
    });
    ...
    it('renders_avg_response_time', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('chat-avg-response-time')).toBeInTheDocument();
      });

      expect(screen.getByTestId('chat-avg-response-time')).toHaveTextContent('850ms');
    });
    ...
    it('formats_response_time_in_seconds_when_over_1000ms', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('chat-avg-response-time')).toHaveTextContent('2.5s');
      });
    });
  });
  ...
  describe('Edge cases', () => {
    it('handles_zero_active_sessions', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('chat-active-sessions')).toHaveTextContent('0');
      });
    });
  });
});
